...
class NunjucksBrunchPlugin {
    ...
    templateFactory(templatePath, options, callback) {
        const env = new nunjucks.Environment(new nunjucks.FileSystemLoader(path.dirname(templatePath)));
        env.render(options.filename, options, callback);
    }
    ...
    compile(data, originalPath, callback) {
        ...
        var relativePath = path.relative(this.projectPath, templatePath);
        var options = _.extend({}, this.nunjucksOptions);
